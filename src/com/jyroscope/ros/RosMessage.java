...
public final class RosMessage {
    ...
    public RosMessage() {
        this(null, DEFAULT_INITIAL_SIZE);
    }
    
    public RosMessage(RosMessageType type) {
        this(type, type == null ? DEFAULT_INITIAL_SIZE : type.getSize());
    }
    
    private RosMessage(RosMessageType type, int initialSize) {
        if (initialSize <= 0)
    ...
    public void zero(int length) {
        checkBounds(length);
        Arrays.fill(buffer, position, length, (byte)0);
        position += length;
    }
