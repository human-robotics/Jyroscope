...
public class RosTypes {
        ...
        if (!types.containsKey(typeName)) {
            try {
                ...
                if (file == null) {
                    ...
                    if (file == null)
                        throw new FileNotFoundException("Msg definition for " + typeName + " not found in " + msgPaths.toString());
                }
    ...
    private static File getMsgFile(String typeName) {
        for (File path : msgPaths) {
            ...
            if (slash != -1) {
                file = new File(new File(path, typeName.substring(0, slash)), "msg" + typeName.substring(slash) + ".msg");
                if (file.exists())
                    return file;
            }
        }
